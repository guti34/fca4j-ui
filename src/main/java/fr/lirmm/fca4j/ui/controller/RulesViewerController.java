...
/**
 * Viewer de règles d'implication (RULEBASIS / DBASIS).
 * Supporte les formats TXT, JSON, XML et Datalog.
 * Affiche les règles avec mise en valeur visuelle et
...
public class RulesViewerController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        sortCombo.getItems().addAll(
            ...
            I18n.get("rules.sort.support.desc"),
            I18n.get("rules.sort.support.asc"),
            I18n.get("rules.sort.premise.size"),
    ...
    /** Format XML : <rule support="N"><premise>...</premise><conclusion>...</conclusion></rule> */
    private List<Rule> parseXml(String content) {
        ...
        for (int i = 1; i < ruleBlocks.length; i++) {
            ...
            try {
                int support = Integer.parseInt(
                    block.substring(block.indexOf("support=\"") + 9,
    ...
    /**
     * Format Datalog : "% support=N\n conclusions(X) :- premises(X)."
     * Nettoie les URIs <http://...> et la variable (X).
     */
    ...
    private void applyFilterAndSort() {
        ...
        filteredRules.addAll(allRules.stream()
            .filter(r -> filter.isEmpty()
                || r.premises().stream().anyMatch(p -> p.toLowerCase().contains(filter))
    ...
    private HBox buildRuleRow(Rule rule, int index) {
        ...
        badge.setStyle("-fx-background-radius: 999; -fx-font-weight: bold;"
            + " -fx-background-color: " + supportColor(rule.support()) + ";"
            + " -fx-text-fill: " + supportTextColor(rule.support()) + ";");
    ...
    private String supportColor(int support) {
        if (support == 0) return "#FECACA";    // rouge pâle
        if (support <= 2) return "#FED7AA";    // orange pâle
        if (support <= 4) return "#FEF08A";    // jaune pâle
        return "#BBF7D0";                       // vert pâle
    }

    private String supportTextColor(int support) {
        if (support == 0) return "#991B1B";
        if (support <= 2) return "#9A3412";
        if (support <= 4) return "#854D0E";
        return "#166534";
    }
    ...
    @FXML
    private void onCopy() {
        if (filteredRules.isEmpty()) return;
        String text = filteredRules.stream()
            .map(Rule::toTxt)
    ...
    private void updateStatus() {
        if (allRules.isEmpty()) {
            ...
            statusLabel.setText(I18n.get("rules.status.filtered",
