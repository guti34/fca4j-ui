...
public class ModelEditorController implements Initializable {
    ...
    @FXML private TableView<FormalContextDef>           contextsTable;
    ...
    @FXML private TableView<AttrRow>            attrsTable;
    @FXML private TableColumn<AttrRow, Number>  attrIdxCol;
    @FXML private TableColumn<AttrRow, String>  attrNameCol;
    @FXML private TableColumn<AttrRow, Boolean> attrKeyCol;
    @FXML private TableColumn<AttrRow, Boolean> attrBinCol;

    // ── Zone relations ────────────────────────────────────────────────────────
    @FXML private TableView<RelationalContextDef>           relationsTable;
    @FXML private TableColumn<RelationalContextDef, String> relNomCol;
    @FXML private TableColumn<RelationalContextDef, String> relSourceCol;
    ...
    private List<String> loadCsvHeaders(FormalContextDef fc) {
        if (fc.path.isBlank()) return List.of();
        Path resolved = resolveCsvPath(fc.path);
    ...
    @FXML private void onBrowseCsv() {
        if (selectedFc == null) return;
    ...
    @FXML private void onClearTransaction() {
        if (selectedRc == null) return;
        selectedRc.path = "";
        relationsTable.refresh();
    ...
    private void editFormalContextMeta(FormalContextDef fc) {
        ...
        nameField.textProperty().addListener((obs, old, val) -> {
            if (val.contains(" ")) nameField.setText(val.replace(" ", "_"));
        });
        ...
        dialog.showAndWait().ifPresent(btn -> {
            ...
            if (!fc.nom.equals(oldName)) {
                for (RelationalContextDef rc : model.getRelationsUsing(oldName)) {
                    ...
                    if (rc.target.equals(oldName)) rc.target = fc.nom;
                }
                relationsTable.refresh();
            }
            contextsTable.refresh();
    ...
    public void loadModel(Path path) {
        try {
            ...
            currentFile = path; modified = false; selectedFc = null; selectedRc = null;
            refreshAll();
    ...
    private void markModified() {
        if (!modified) {
            modified = true;
            Platform.runLater(() -> modelNameLabel.setText(
                (currentFile == null ? I18n.get("model.new.name")
                                     : currentFile.getFileName().toString()) + " *"));
        }
    }
    ...
    @FXML public void onAddContext() {
        ...
        pathField.setPromptText(I18n.get("model.col.path.optional"));
    ...
    private Path resolveCsvPath(String csvPath) {
        ...
        if (currentFile != null) return currentFile.getParent().resolve(p);
        return null;
    }
    ...
    private boolean confirmDelete(String msg) {
        Alert a = new Alert(Alert.AlertType.CONFIRMATION);
        a.setTitle(I18n.get("editor.confirm.delete.title")); a.setHeaderText(null);
